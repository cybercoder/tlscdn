...
func OnAddHTTPRoute(obj interface{}) {
	...
	err = redisClient.Set(context.Background(),
		...
		jsonData, 0).Err()
	if err != nil {
		log.Printf("Error on storing httproute: %s to redis: %v", redisKey, err)
		return
	}

	err = unstructured.SetNestedField(u.Object, redisKey, "status", "redisKey")
	if err != nil {
		log.Printf("failed to set redis key on httproute %s status:", httproute.GetName(), err)
		return
	}
	_, err = k.Resource(v1alpha1.HTTPRouteGVR).Namespace(httproute.GetNamespace()).UpdateStatus(context.TODO(), u, metav1.UpdateOptions{})
	if err != nil {
		log.Printf("failed to update httproute %s status:", httproute.GetName(), err)
		return
	}
}
